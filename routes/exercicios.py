...
def add_exercicio_to_treino(
    treino_id: int, exercicio_id: int, session: Session = Depends(get_session)
):
    ...
    link_existente = (
        session.query(TreinoExercicioLink)
        .filter_by(treino_id=treino_id, exercicio_id=exercicio_id)
        .first()
    )
    if link_existente:
        raise HTTPException(
            status_code=400, detail="Exercício já associado a este treino"
        )
