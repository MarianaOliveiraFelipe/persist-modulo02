...
def get_alunos_por_peso(peso_minimo: float, session: Session):
    ...
    statement = select(Aluno).where(Aluno.peso > peso_minimo).options(selectinload(Aluno.treinos))
    alunos = session.exec(statement).all()

    if not alunos:
        raise HTTPException(status_code=404, detail="Nenhum aluno encontrado com peso superior a esse valor.")
...
def get_alunos_por_altura(altura_minima: float, session: Session):
    ...
    statement = select(Aluno).where(Aluno.altura > altura_minima).options(selectinload(Aluno.treinos))  
    alunos = session.exec(statement).all()

    if not alunos:
        raise HTTPException(status_code=404, detail="Nenhum aluno encontrado com altura superior a esse valor.")
