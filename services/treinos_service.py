...
def count_treinos_por_aluno(session: Session):
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Erro ao carregar os treinos por aluno: {str(e)}")
...
def get_treinos_por_dia(session: Session, dia_semana: str):
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Erro ao carregar os treinos para o dia {dia_semana}: {str(e)}")
...
def count_treinos_por_dia(session: Session) -> list[tuple]:
    try:
        statement = (
            select(Treino.dia_semana, func.count(Treino.id).label("quantidade"))
        ...
        raise HTTPException(status_code=500, detail=f"Erro ao carregar a contagem de treinos por dia: {str(e)}")
